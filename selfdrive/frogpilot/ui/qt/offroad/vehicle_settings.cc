...
QStringList getCarNames(const QString &carMake, QMap<QString, QString> &carModels) {
  ...
  QRegularExpression carModelRegex(R"delimiter((\w+)\s*=\s*(\w+PlatformConfig)\s*\([^)]*\))delimiter");
  ...
  if (carDocsClassMatch.hasMatch()) {
    ...
    QRegularExpression footnoteRegex(QString(R"delimiter(%1\("([^"]+)",[\s\S]*footnotes=)delimiter").arg(carDocsClassName));
...
void FrogPilotVehiclesPanel::updateCarToggles() {
  ...
  if (!carParams.empty()) {
    ...
    isGMTruck = carFingerprint == "CHEVROLET_SILVERADO";
