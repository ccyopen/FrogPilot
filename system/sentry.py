...
def format_params(params):
  return [f"{key}: {value.decode() if isinstance(value, bytes) else format(value, '.12g') if isinstance(value, float) else value}"
          for key, value in sorted(params.items())]


def get_frogpilot_params_by_type(param_type, params):
  return {key: (params.get(key).decode() if isinstance(params.get(key), bytes) else params.get(key) or '0')
